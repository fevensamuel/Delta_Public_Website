...
export const FloatingWhatsApp: React.FC = () => {
  ...
  const { whatsappNumber, phoneNumber, smsNumber } = useContactSettings();

  const cleanWhatsApp = (whatsappNumber || '').replace(/[^0-9]/g, '');
  const cleanPhoneNum = (phoneNumber || '').replace(/[^0-9+]/g, '');
  const cleanSmsNum = (smsNumber || '').replace(/[^0-9+]/g, '');

  const handleSend = (text?: string) => {
    const finalMsg = encodeURIComponent(
      text ||
        userMsg ||
        ' I would like to inquire about Delta Travel Umrah packages.'
    );
    window.open(`https://wa.me/${cleanWhatsApp}?text=${finalMsg}`, '_blank');
    setUserMsg('');
    setIsOpen(false);
  };
  ...
  return (
    <div className="fixed right-0 top-1/2 -translate-y-1/2 z-50 flex flex-col items-end gap-2 pr-0.5">
            ...
            <div className="p-4 bg-[#FAF7F2] space-y-3 text-xs">
              <div className="bg-white p-3 shadow-sm border border-black/[0.08]">
                <p className="font-medium text-[#1A1712]">
                  Assalamu Alaikum! 🕋 How can Delta Travel & Tour assist your
                  sacred journey today?
                </p>
              </div>
              ...
              <div className="space-y-1.5 pt-1">
                ...
                <button
                  onClick={() =>
                    handleSend('Inquiry: Umrah E-Visa Processing Time & Documents')
                  }
                  className="w-full text-left p-2 bg-white hover:bg-[#F1EBE0] text-[#1A1712] border border-black/[0.08] transition-colors flex items-center justify-between"
                >
      ...
      <div className="flex flex-col items-end space-y-1.5 font-sans text-xs">
        ...
        <a
          href={`sms:${cleanSmsNum}`}
          className="bg-[#7A0C1F] hover:bg-[#580815] text-white py-2.5 px-3 rounded-l-md flex flex-col items-center justify-center min-w-[70px] h-[60px] transition-transform hover:-translate-x-1 border-y border-l border-[#580815]"
          title="Send SMS"
        >
